...
public class UserController {
	...
	public String create(Locale locale,HttpServletRequest request) throws ParseException {
	...
	@RequestMapping(value="user/checkDuplicateEmailAjax" , method = RequestMethod.POST)
	    ...
	    boolean isDuplicateEmail = this.userService.checkEmail(create_email);
	        /**
	         * Response로 응답하기 위해서 PrintWriter객체를 생성한다.
	         */
	        AjaxUtil.sendResponse(response, isDuplicateEmail+"");
	    }
 /*	@RequestMapping(value="/user/captchaAjax" , method = RequestMethod.POST)
    public ModelAndView checkDuplicateCaptcha(HttpServletRequest request, HttpServletResponse response) throws IOException{
		
		String secretKey = "6LdPxigTAAAAALnuj96AOCIS2PMEglPhEsAci9ds";
