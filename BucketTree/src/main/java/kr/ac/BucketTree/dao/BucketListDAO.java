...
public interface BucketListDAO {
	...
	/* 전체 버킷리스트 백승호 수정 */
	public List<BucketListVO> list(Pagination pagination, int user_idx)
			throws Exception;/* 전체 목록 및 정렬,검색 */ /* 버킷리스트-무한스크롤 */

	int listCount(Pagination pagination, int user_idx) throws Exception;

	public int listImage(int bucket_idx)
			throws Exception; /* 버킷리스트-첫번째 이미지로 보이기 */

	public void countUp(int idx) throws Exception; /* 담기_카운트 업 */

	public void addBucket(BucketListVO vo)
			throws Exception; /* 버킷 버튼 클릭 -> 마이 버킷에 추가 */

	public boolean titleCheck(String mtitle, int userIdx)
			throws Exception; /* 담아올 버킷이 마이 버킷리스트에 있는지 중복 타이틀 검사 */
	/* 마이 버킷리스트 백승호 수정 */
	...
	int mylistCount_Complete(int user_idx) throws Exception;
